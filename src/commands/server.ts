...
👥 在线玩家: ${data.onlinePlayers}

⏰ 时间信息:
  • 白天时间: ${data.time.daytime}
  • 游戏时间: ${data.time.gametime}
  • 游戏天数: 第 ${data.time.day} 天
...
#block(
  ...
  #text(weight: "bold", fill: ${theme.sectionTitle})[⏰ 时间信息]
  #v(6pt)
  #table(
    columns: (auto, 1fr),
    stroke: none,
  ...
  ctx.command('mcinfo.server', '服务器详细信息')
    ...
    .action(async ({ session, options }) => {
      try {
        ...
        return `❌ 获取服务器详细信息失败: ${error.message}`
      }
    })
}
