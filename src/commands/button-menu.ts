...
export function registerButtonMenuCommand({
  ...
  ctx.command(
    ...
    .action(async ({ session }, requestedPage) => {
      if (session.platform !== 'qq') return withQuote(session, config, '❌ 按钮菜单仅支持 QQ 平台')
      if (!config.qqKeyboardEnabled) return withQuote(session, config, '❌ QQ 按钮已关闭，无法发送按钮菜单')

      const page = requestedPage === undefined ? 1 : Number(requestedPage)
      if (!Number.isInteger(page)) return withQuote(session, config, `❌ 页码只能是 1 或 ${BUTTON_MENU_PAGE_COUNT}`)
      if (page < 1) return withQuote(session, config, '❌ 已经是第一页了')
      if (page > BUTTON_MENU_PAGE_COUNT) return withQuote(session, config, '❌ 已经是最后一页了')

      const menu = buildQQButtonMenu(config, page as 1 | 2)
      if (!menu.keyboard) return withQuote(session, config, '❌ QQ 按钮已关闭，无法发送按钮菜单')
      try {
        ...
        return withQuote(session, config, `❌ 发送按钮菜单失败：${error instanceof Error ? error.message : String(error)}`)
      }
    })
}
